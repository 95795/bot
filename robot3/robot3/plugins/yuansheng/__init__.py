...
from nonebot import on_command,on_keyword, logger, require
...
what2eat = on_keyword({'原批','原p'},priority=15, block=True)
group_add = on_command("添加op语录", priority=15, block=True)
group_remove = on_command("移除", permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=15, block=True)
basic_add = on_command("添加", permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=15, block=True)
...
add_greeting = on_command("添加问候", aliases={"添加问候语"}, priority=12, block=True)
...
async def _(event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not args_str:
        await group_add.finish("没活可以去咬打火机")
    elif len(args_str) > 1:
        await group_add.finish("参数错误")
...
async def _(args: Message = CommandArg()):
    args_str: List[str] = args.extract_plain_text().strip().split()
    if not args_str:
        await basic_add.finish("没活可以去咬打火机")
...
async def _(event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not args:
        await group_remove.finish("还没输入你要移除的语录")
    elif len(args) > 1:
        await group_remove.finish("移除错误")
